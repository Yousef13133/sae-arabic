...
Usage:
  ...
  python -m scripts.causal_validate --checkpoint data/real_run/checkpoints/final.pt --layer 6 --num-texts 200 --num-features 5
"""
...
def main() -> None:
    ...
    parser.add_argument("--num-texts", type=int, default=200)
    ...
    print(f"\n{'feature':>8} {'ablate Δ':>12} {'amplify Δ':>12} {'control Δ':>12}")
    for fid in top_feat_ids:
        abl = feature_effects.get(f"ablate:{fid}", {}).get("delta", 0)
        amp = feature_effects.get(f"amplify:{fid}", {}).get("delta", 0)
        ctrl = feature_effects.get(f"control:{fid}", {}).get("delta", 0)
        print(f"{fid:>8} {abl:>+12.4f} {amp:>+12.4f} {ctrl:>+12.4f}")

    print(f"\nbaseline={baseline_mean:.4f}  base_recon={base_mean:.4f}  causal_delta_range=[{min(e['delta'] for e in feature_effects.values()):.4f}, {max(e['delta'] for e in feature_effects.values()):.4f}]")
